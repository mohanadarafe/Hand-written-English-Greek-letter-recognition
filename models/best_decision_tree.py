...
def best_dt(dataset):
    ...
    print()
    print(f"-"*60)
    print(f"Running Best decision tree on the {language} alphabet")
    ...
    param_grid = {
        ...
        'max_depth': [None, 10],
        'min_samples_split': [11, 15, 27, 29, 41, 49, 55],  # we decide this value
        'min_impurity_decrease': [0.0, 0.1, 0.5, 1],  # we decide this value
    ...
    utils.generate_report(dataset['nolabel'], outputdir, model, label_matrix, dataset, (train_acc ,val_acc, test_acc, macrof1, weightf1), "Best Decision Tree")
